...
۲. **دانلود پروژه:**
   ...
   ```
۳. **اجرای اسکنر:** (برنامه پیش‌نیازها و هسته لینوکسی Xray را خودش دانلود و نصب می‌کند!)
   ```bash
   python3 scanner.py
   ```
...
* **مکانیسم:** از `asyncio.open_connection` پایتون برای ارسال صدها درخواست سوکت همزمان استفاده می‌شود. Worker دقیقاً در همان میلی‌ثانیه‌ای که بسته `SYN-ACK` دریافت می‌شود، سوکت را می‌بندد.

### مرحله ۲: تزریق TLS SNI (لایه ۷)
* **هدف:** تأیید رمزنگاری‌شده‌ی تعلق آی‌پی به کلودفلر و دور زدن مسدودسازی دامنه‌ها مبتنی بر SNI.
* **مکانیسم:** موتور یک `ssl.create_default_context()` تأیید نشده می‌سازد و یک تونل امن برقرار می‌کند، و یک دامنه "سفید" را به هدر SNI تزریق می‌کند. اگر هدرهای پاسخ حاوی امضاهای کلودفلر باشد، IP به عنوان یک نقطه ورود معتبر تأیید می‌شود.
...
* **مکانیسم:** موتور به صورت پویا پارامترهای غیرضروری را حذف می‌کند تا از کرش کردن `geosite.dat` جلوگیری کند. سپس یک نمونه ایزوله از `Xray-core` را در پس‌زمینه اجرا کرده و یک اتصال زنده را از طریق سرور تست هدایت می‌کند تا پینگ دقیق و رمزنگاری‌شده (TTFB) را محاسبه کند.

---

## 🧠 بهینه‌سازی‌های الگوریتمی در پس‌زمینه

* **محدودیت‌های همزمانی هوشمند:** موتور برنامه با خواندن `os.cpu_count()` سقف ایمن سوکت‌ها را محاسبه می‌کند (محدودیت ۱۰۰۰ سوکت در ویندوز، اما تا ۳۰۰۰ سوکت در لینوکس/مک‌اواس با استفاده از مکانیزم‌های پیشرفته).
...
* **حلقه بازخورد "ساب‌نت‌های داغ":** هنگامی که یک آی‌پی موفق می‌شود، برنامه آن بلوک `/24` را به خاطر می‌سپارد و موقتاً تمرکز را روی آن منطقه افزایش می‌دهد.
* **کاهش فشار ناهمگام:** صف‌های `asyncio.Queue` محدود شده از مصرف بیش از حد رم جلوگیری می‌کنند و در صورت شلوغی بیش از حد Xray، آی‌پی‌های اضافی را دور می‌اندازند.
* **تجزیه دوطرفه پیکربندی:** موتور قدرتمند RegEx که فایل‌های تو در توی JSON و لینک‌های متنی (`vless://`) را در لحظه به یکدیگر تبدیل می‌کند.
...
۱. **لاگر حرفه‌ای:** روی دکمه **"Save Log"** در داشبورد ترمینال کلیک کنید. ماژول `logging` به طور خاموش در پس‌زمینه اجرا می‌شود و خروجی‌ها، تایم‌اوت‌های سوکت، و خطاهای هسته Xray را ثبت می‌کند.
۲. **بررسی `scanner_error.log`:** این فایل را در پوشه اصلی برنامه بررسی کنید.
۳. **باز کردن Issue:** به برگه [GitHub Issues](https://github.com/amirrezas/WaldonCFscanner-python/issues) بروید و فایل لاگ خود را پیوست کنید. مشارکت‌های برنامه‌نویسان دیگر به شدت استقبال می‌شود!
